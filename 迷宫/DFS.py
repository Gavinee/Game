...
def GameOver(chr1,size1):
    ...
    turtle.penup()
    turtle.goto(0,0)
    turtle.write(chr1,("Times",size1,"bold"))
    turtle.pendown()
...
def route(path,deviceDistance,deviceX,deviceY):             #需要走过的路径
    ...
    for i in path:
        ...
        drawRectangle(color, fillingColor, pensize, speed, i, deviceDistance)
        q = 10000
        while (q > 0):
            q = q - 1

def main():
    deviceX = 350                           #屏幕上X轴正半轴的边界
    deviceY = 350                           #屏幕上Y轴正半轴的边界
    deviceDistance = 50                     #每个迷宫方格的间距
